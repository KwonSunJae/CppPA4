...
void Game::play_game()
{
	...
	while (1)
	{
		...
		for (i = 1; 1; i++)
		{
			...
			{
				console.Color(RED);
				cout << is_gameover << " " << i << " " << sk2_time << " " << sk5_time << endl;
				system("pause");
				show_gameover();
...
int Game::show_total_block()
{
	...
	for (i = 0; i < game_map[level].size_y + 1; i++)
	{
		for (j = 0; j < game_map[level].size_x + 2; j++)
		{
			...
			console.gotoxy((j * 2) + ab_x - 1, i + ab_y + 1);
			if (game_map[level].map[i][j] > 0)
			{
				if ((level == 4 || level == 7) && game_map[level].map[i][j] == 1 && game_map[level].skill == 4) cout << "  ";
				else cout << "■";
			}
			else {
				printf("  ");
...
int Game::strike_check(int shape, int angle, int x, int y)
{
	...
	for (i = 0; i < 4; i++)
	{
		for (j = 0; j < 4; j++)
		{
			if (((x + j) == game_map[level].size_x + 1) || (x + j) == 0)
				block_dat = 1;
			else
				block_dat = game_map[level].map[y + i][x + j];

			if (block_dat <= 2 && block_dat > 0 && (block.block[shape][angle][i][j] == 1 || y < -2)) //좌측벽의 좌표를 빼기위함, FIXED_6 왼쪽에 일자블록 나오자마자 집어넣으면 Game over 뜨는 오류 수정
			{
				return 1;
			}
		}
